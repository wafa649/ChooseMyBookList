...
// Fonction de rappel pour afficher les données de la requête SQL
...
// Fonction pour obtenir le nom du livre par ID depuis la base de données
...
// Fonction pour charger la liste des livres à partir d'un fichier
void loadBookList() {
...
int main() {
    ...
    loadBookList(); // Charger la liste des livres à partir du fichier
    ...
    while (1) {
        ...
        if (strcmp(commande, "show") == 0) {
            ...
            getBookNameByID(db);
            saveBookList(); // Sauvegarder la liste des livres dans un fichier
